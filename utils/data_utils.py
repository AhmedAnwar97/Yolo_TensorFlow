...
def parse_line(line):
    ...
    if 'str' not in str(type(line)):
        line = line.decode()
    s = line.strip().split(' ')
    assert len(s) > 20, 'Annotation error! Please check your annotation file. Make sure there is at least one target object in each image.'
    line_idx = int(s[0])
    pic_path = s[1]
    img_width = int(s[2])
    img_height = int(s[3])
    s = s[4:]
    assert len(s) % 19 == 0, 'Annotation error! Please check your annotation file. Maybe partially missing some coordinates?'
    box_cnt = len(s) // 19
    boxes = []
    labels = []
    keyPoints = []
    for i in range(box_cnt):
        label, x_min, y_min, x_max, y_max , keyPoint = int(s[i * 19]), float(s[i * 19 + 1]), float(s[i * 19 + 2]), float(
            s[i * 19 + 3]), float(s[i * 19 + 4]) , [ float(x) for x in s[ i*19+5 : i*19 + 19] ]
    ...
    return line_idx, pic_path, boxes, labels, img_width, img_height, keyPoints


def process_box(boxes, labels, img_size, class_num, anchors, keyPoints):
    '''
    ...
    y_true_13 = np.zeros((img_size[1] // 32, img_size[0] // 32, 3, 20 + class_num), np.float32)
    y_true_26 = np.zeros((img_size[1] // 16, img_size[0] // 16, 3, 20 + class_num), np.float32)
    y_true_52 = np.zeros((img_size[1] // 8, img_size[0] // 8, 3, 20 + class_num), np.float32)
    ...
    for i, idx in enumerate(best_match_idx):
        ...
        y_true[feature_map_group][y, x, k, 5 + class_num : 5+class_num+14 ]  = keyPoints[i]
...
def parse_data(line, class_num, img_size, anchors, mode, letterbox_resize):
    ...
    if not isinstance(line, list):
        img_idx, pic_path, boxes, labels, _, _ , keyPoints = parse_line(line)
        img = cv2.imread(pic_path)
    #     # expand the 2nd dimension, mix up weight default to 1.
    #     boxes = np.concatenate((boxes, np.full(shape=(boxes.shape[0], 1), fill_value=1., dtype=np.float32)), axis=-1)
    # else:
    #     # the mix up case
    #     _, pic_path1, boxes1, labels1, _, _,keyPoints1 = parse_line(line[0])
    #     img1 = cv2.imread(pic_path1)
    #     img_idx, pic_path2, boxes2, labels2, _, _ ,keyPoints2 = parse_line(line[1])
    ...
    y_true_13, y_true_26, y_true_52 = process_box(boxes, labels, img_size, class_num, anchors, keyPoints)
